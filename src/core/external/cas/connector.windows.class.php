...
class stack_cas_connection_windows extends stack_cas_connection_base {
    ...
    protected function guess_maxima_command($path) {
        if ('default' == stack_connection_helper::get_maximaversion()) {
            throw new StackException("stack_cas_connection: maxima cannot be set to default on Windows platform. ".
                    "Please choose an explicit version via the administration settings page.");
        }
        $cmd = $path . '/maxima.bat';
        if (!is_readable($cmd)) {
            throw new StackException("stack_cas_connection: maxima launch script {$cmd} does not exist.");
        }
        return $cmd;
    }
    ...
    protected function call_maxima($command) {
        ...
        if (!is_resource($casprocess)) {
            throw new StackException('stack_cas_connection: Could not open a CAS process.');
        }
