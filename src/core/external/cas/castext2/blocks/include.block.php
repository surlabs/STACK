...
class stack_cas_castext2_include extends stack_cas_castext2_block {
    ...
    public function compile($format, $options): ?MP_Node {
        ...
        if (isset($options['in include'])) {
            ...
            throw new StackException('CASText2 inclusions within inclusions are not currently supportted, ' .
                'due to security validation logic: ' . $this->params['src']);
        }
        if ($src === false) {
            throw new StackException('Include block source not accessible: ' . $this->params['src']);
        }
