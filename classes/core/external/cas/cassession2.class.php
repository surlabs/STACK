...
class stack_cas_session2 {
    ...
    public function __construct(array $statements, $options = null, $seed = null) {
        ...
        if (!($seed === null)) {
            if (is_int($seed)) {
                ...
                throw new StackException('stack_cas_session: $seed must be a number.  Got "'.$seed.'"');
            }
        } else {
            $this->seed = time();
        }
    ...
    public function instantiate(): bool {
        if (!$this->get_valid()) {
            throw new StackException('stack_cas_session: cannot instantiate invalid session');
        }
            ...
            foreach ($results['values'] as $key => $value) {
                if (is_string($value)) {
                    try {
                        ...
                        throw new StackException('stack_cas_session: tried to parse the value ' .
