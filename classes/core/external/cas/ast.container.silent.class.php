...
use Exception;
use classes\core\external\cas\castext2\parsingrules\stack_parsing_rule_factory;
use classes\core\external\maximaparser\maxima_corrective_parser;
use classes\core\external\maximaparser\maxima_parser_utils;
...
use classes\core\external\maximaparser\MP_Integer;
use classes\core\external\maximaparser\MP_List;
use classes\core\external\maximaparser\MP_Operation;
use classes\core\external\maximaparser\MP_PrefixOp;
use classes\core\external\maximaparser\MP_Root;
use classes\core\external\maximaparser\MP_Set;
...
class stack_ast_container_silent implements cas_evaluatable {
    ...
    public function get_evaluationform(): string {
        if (false === $this->get_valid()) {
            throw new StackException('stack_ast_container: tried to get the evaluation form of an invalid casstring.');
        }
    ...
    public function get_inputform(bool $keyless = false, $nounify = null, $nontuples = false,
        ...
        if (!($nounify === null || is_int($nounify))) {
            throw new StackException('stack_ast_container: nounify must be null or an integer.');
        }
        if (!($decimals === '.' || $decimals === ',')) {
            throw new StackException('stack_ast_container: decimal option must be "." or ",".');
        }
        $decimal = '.';
        $listsep = ',';
        if ($decimals == ',') {
    ...
    public function ast_to_string($root = null, $parameters = array()) : string {
        ...
        // throw new StackException('stack_ast_container::ast_to_string tried to set illegal parameter ' . $key);
    ...
    public function get_feedback($raw = 'implode') {
        ...
        if ($raw === 'implode') {
            ...
            foreach ($this->feedback as $fb) {
                $feedback[trim(StackUtils::stackMaximaTranslate($fb))] = true;
            }
