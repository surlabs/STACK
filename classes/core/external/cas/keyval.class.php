...
class stack_cas_keyval {
    ...
    public function __construct($raw, $options = null, $seed=null, $ctx='') {
        ...
        if (!is_null($seed) && !is_int($seed)) {
            throw new StackException('stack_cas_keyval: seed must be a null or an integer.');
        }
    ...
    private function validate($inputs) {
        ...
        if (!$ast instanceof MP_Root) {
            ...
            if ($ast instanceof StackException) {
    ...
    public function compile(string $contextname, castext2_static_replacer $map = null): array {
        ...
        if (null === $this->valid) {
            throw new StackException('stack_cas_keyval: must validate before compiling.');
        }
        if (false === $this->valid) {
            throw new StackException('stack_cas_keyval: must validate true before compiling.');
        }
