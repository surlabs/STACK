...
class stack_cas_castext2_ascii extends stack_cas_castext2_block {
    ...
    public function compile($format, $options): ?MP_Node {
        ...
        // Keep a configured minimum after applying the stylesheet offsets.
        $astyle = "width:calc({$xpars['width']} - var(--stack-ascii-horizontal-offset));";
        $astyle .= "height:calc(100vh - var(--stack-ascii-vertical-offset));";
        $astyle .= "min-height:calc({$xpars['height']} - var(--stack-ascii-vertical-offset));";
        if (array_key_exists('aspect-ratio', $xpars)) {
            ...
            if ($existsuserheight) {
                $astyle = "height:calc(100vh - var(--stack-ascii-vertical-offset));aspect-ratio:$aspectratio;";
                $astyle .= "min-height:calc({$xpars['height']} - var(--stack-ascii-vertical-offset));";
            } else if ($existsuserwidth) {
                $astyle = "width:calc({$xpars['width']} - var(--stack-ascii-horizontal-offset));aspect-ratio:$aspectratio;";
            }
        }
