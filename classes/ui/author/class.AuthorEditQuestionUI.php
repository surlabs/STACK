...
class AuthorEditQuestionUI
{
    ...
    public function showAuthoringPanel()
    {
        ...
        if ($this->getQuestionGUI()->object->getQuestion() == "") {
            ...
            $this->getQuestionGUI()->object->setPoints(1.0);
        }
    ...
    public function addInputs()
    {
        ...
        if (!empty($this->getQuestionGUI()->object->inputs)) {
            ...
            foreach ($this->getQuestionGUI()->object->inputs as $input_name => $input) {
                ...
                $input_part->setTitle($this->getPlugin()->txt('auth_inputs') . " " . $input_name);
                $inputs->addPart($input_part);
            }
        } else {
            //load standard input
            $standard_input = StackConfig::get('inputs');
            ...
            $input_part = $this->getInputPart($all_parameters);
...
*/
     ...
     * @return ilMultipartFormPart
     */
    public function getInputPart(array $data): ilMultipartFormPart
    {
        ...
        if ($this->new_question) {
            ...
            $input_model_answer->setValue($this->default["input_model_answer"]);
            ...
            $input_type->setValue($data["input_type"]);
            $input_model_answer->setValue($data["input_model_answer"]);
            $input_box_size->setValue($data["input_box_size"]);
            $input_strict_syntax->setChecked((bool)$data["input_strict_syntax"]);
            $input_insert_stars->setValue((int)$data["input_insert_stars"]);
            $input_syntax_hint->setValue($data["input_syntax_hint"]);
            $input_forbidden_words->setValue($data["input_forbidden_words"]);
            $input_allow_words->setValue($data["input_allow_words"]);
            $input_forbid_float->setChecked((bool)$data["input_forbid_float"]);
            $input_require_lowest_terms->setChecked((bool)$data["input_require_lowest_terms"]);
            $input_check_answer_type->setChecked((bool)$data["input_check_answer_type"]);
            $input_must_verify->setChecked((bool)$data["input_must_verify"]);
            $input_show_validation->setValue((int)$data["input_show_validation"]);
            $input_options->setValue($data["input_extra_options"]);
        }
